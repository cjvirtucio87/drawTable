...
function padLine (line,padding,option) {
...
TextCell.prototype.draw = function (width,height) {
	...
	for (var i = 0; i < height; i++) {
		...
		result.push(padLine(line,padding,true));
	}
	return result;
}
...
//Left-padded cell for numbers.
function LPadCell (text) {
	TextCell.call(this,text);
}

LPadCell.prototype = Object.create(TextCell.prototype);

LPadCell.prototype.draw = function (width,height) {
	var result = [];
	for (var i = 0; i < height; i++) {
		var line = this.text[i];
		var padding = width - line.length;
		result.push(padLine(line,padding,false));
	}
	return result
}
...
StretchCell.prototype.draw = function (width,height) {
	var result = [];
	var innerHeight = this.inner.text.length;
	for (var i = 0; i < innerHeight; i++) {
		var line = this.inner.text[i];
		var padding = width - line.length;
		padLine(line,padding,true);
		result.push(line);
	}
	while (result.length < height) {
		result.push(padLine("",width,true));
	}
	return result;
}
...
//App.
//Convert a string to a cell object, and a number
//to a LPadCell object.
function convertToCell (data) {
	return data.match(/\b\d+\b/) ? new LPadCell(data) :
								   new TextCell(data);
}
...
function makeTableHeaders (keys) {
	return keys.map(function(key) {
		return new UnderlinedCell(convertToCell(key));
	});	
}
...
function makeTableBody (dataObjects,keys) {
	return dataObjects.map(function(object) {
		return keys.map(function(key) {
			var cell = (object[key])
			return convertToCell(cell.toString());
		});
	});
}
